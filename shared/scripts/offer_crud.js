...
function buildJsonFromFormInput(data) {
  ...
  for (let index = 0; index < currentOffer.details.length; index++) {
    const offerType = currentOffer.details[index].offer_type;

    currentOffer.details[index].title = data[`offer_title_${offerType}`];
    currentOffer.details[index].delivery_time_in_days = Number(data[`offer_delivery_time_${offerType}`]);
    currentOffer.details[index].price = Number(data[`offer_price_${offerType}`]);

    if (data[`offer_revisions_${offerType}_limitless`]) {
      currentOffer.details[index].revisions = 0;
    } else {
      currentOffer.details[index].revisions = Number(data[`offer_revisions_${offerType}`]);
    }
  }
...
function checkRevisionInput(id) {
  ...
  const value = parseInt(inputRef.value, 10);
